...
(function (window, document, Math) {
	...
	function DiaboSlider (el, options) {
		...
		var touchMoveHandler = function(e){
			...
			dslider.desiredIndex = calculateIndex(y);
		};
		/**
		...
		var calculateIndex = function(pos){
			...
			cindex = Math.floor(((dslider.zeroPosition.y + dslider.itemSize.height*options.offsetNum) - pos)/dslider.itemSize.height);
		...
		var moveTo = function(end, dir){	
			...
			console.log(end + ", " + desiredPos)
				if(usableEvent){
				dslider.scroller.style[usableEvent.cssName] = "top " + options.duration/1000 + "s ease-out";
